    ...
    for line in text.splitlines():
        ...
        if header_match:
            ...
            if month_name in MONTHS:
                current_month, current_year = MONTHS[month_name], year
            continue
        entry_match = ENTRY_LINE.match(line)
        if entry_match and current_month:
            month_name, day, entry_text = entry_match.groups()
    ...
    for filename in os.listdir(NOTES_DIR):
        ...
        for date, entry_text in parse_diary(text):
            if date in already_ingested:
                continue  # already embedded in a previous run — skip

            for i, chunk in enumerate(chunk_text(entry_text)):
                embedding = embed(chunk)
                chunk_id = f"{date}-{i}"  # deterministic: safe to upsert
                collection.upsert(
                    ids=[chunk_id],
                    embeddings=[embedding],
                    documents=[chunk],
                    metadatas=[{"date": date, "source": filename}],
                )
                new_count += 1
            already_ingested.add(date)

    save_manifest(already_ingested)
    print(f"Ingested {new_count} new chunks. Total dates tracked: {len(already_ingested)}")
